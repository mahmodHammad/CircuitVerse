...
const commandsTable ={
    SelectMulti, undo, Copy, Cut, Past, ZoomIn, ZoomOut, delete_selected, SelectAll, save, saveOffline, ChangeBitWidth, ChangeClockTime, RotateLeft, RotateRight, RotateUp, RotateDowm
   }
//***MOUSE-START***
...
function handleMouseDown(e) {
  ...
  simulationArea.mouseDownX =
    Math.round(
      (simulationArea.mouseDownRawX - globalScope.ox) / globalScope.scale / unit
    ) * unit;
  simulationArea.mouseDownY =
    Math.round(
      (simulationArea.mouseDownRawY - globalScope.oy) / globalScope.scale / unit
    ) * unit;
...
function handleKeyUp(e) {
  ...
  if (e.keyCode == 16) simulationArea.shiftDown = false;

  if (e.key == "Meta" || e.keyCode == 17) simulationArea.controlDown = false;

  // select multi buttom (m)
  if (e.keyCode == 69) simulationArea.shiftDown = false;
}
// Key DOWM
function handleKeyDowm(e) {
  if (wait) {
    ...
    //   execute the shortcut on the simulator (if match)
    if (
      simulationArea.mouseRawX < 0 ||
      simulationArea.mouseRawY < 0 ||
      simulationArea.mouseRawX > width ||
    ...
    if (simulationArea.lastSelected) {
      if (
        ...
        (e.key.toString().length == 1 || e.key.toString() == "Backspace")
      ) {
      ...
      if (
        ...
        e.key.toString() != "Backspace" && e.key.toString() != "Delete"
      ) {
    ...
    let found = false;
    //flage to stop looptin (this would make us sure that ther's only one command executed for each key ,if there's somehow multible keys for same command)


function executeCommand(command){
    commandsTable[command]()
    return true
} 
      ...
      instruction[0].forEach(key => {
        if (key === e.keyCode) {
          if (
            ...
            // CTRL + SHIFT pressed
            found = executeCommand(instruction[3])
            return;
...
function handleNewDirection(direction) {
  if (simulationArea.lastSelected != undefined)
    if (direction) simulationArea.lastSelected.newDirection(direction);
}
